...
The `home-routes` handler in the `study-timer.routes.home` namespace
...
The `home-page` function will in turn call the `study-timer.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/study_timer/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `study-timer.handler` namespace:
...
Request middleware functions are located under the `study-timer.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `study-timer.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `study-timer.db.core` in some other namespace.
